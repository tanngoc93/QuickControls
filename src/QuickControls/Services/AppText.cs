    ...
    public static class AppText
    {
        ...
        private static Dictionary<string, string> CreateEnglish()
        {
            return new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                { "Tray.HardwareMonitor", "Hardware monitor" },

                { "Hardware.WindowTitle", "Hardware Monitor — Quick Controls" },
                { "Hardware.Title", "Hardware Monitor" },
                { "Hardware.Intro", "Live performance and temperature at a glance" },
                ...
                { "Hardware.TemperatureUnavailable", "Not reported" },
                { "Hardware.NotDetected", "Not detected" },
                { "Hardware.WarmingUp", "Reading hardware data…" },
                { "Hardware.History", "60-second history" },
                { "Hardware.LastUpdated", "Updated {0}" },
                { "Hardware.SensorNote", "Temperatures appear when supported by the hardware and driver" },
                { "Hardware.MemoryUsed", "{0} of {1} used" },
                { "Hardware.Live", "LIVE" },
                { "Hardware.Open.Accessible", "Open hardware monitor" },
                { "Hardware.Minimize.Accessible", "Minimize hardware monitor" },
                { "Hardware.Close.Accessible", "Close hardware monitor" },
        ...
        private static Dictionary<string, string> CreateVietnamese()
        {
            return new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                { "Tray.HardwareMonitor", "Theo dõi phần cứng" },

                { "Hardware.WindowTitle", "Theo dõi phần cứng — Quick Controls" },
                { "Hardware.Title", "Theo dõi phần cứng" },
                { "Hardware.Intro", "Xem nhanh hiệu năng và nhiệt độ theo thời gian thực" },
                { "Hardware.Cpu", "Bộ xử lý" },
                { "Hardware.Gpu", "Đồ họa" },
                { "Hardware.Memory", "Bộ nhớ" },
                { "Hardware.Storage", "Ổ lưu trữ" },
                { "Hardware.Usage", "MỨC DÙNG" },
                { "Hardware.Temperature", "Nhiệt độ" },
                { "Hardware.TemperatureUnavailable", "Không được báo" },
                { "Hardware.NotDetected", "Không phát hiện" },
                { "Hardware.WarmingUp", "Đang đọc dữ liệu phần cứng…" },
                { "Hardware.History", "Lịch sử 60 giây" },
                { "Hardware.LastUpdated", "Cập nhật lúc {0}" },
                { "Hardware.SensorNote", "Nhiệt độ hiện khi phần cứng và trình điều khiển hỗ trợ" },
                { "Hardware.MemoryUsed", "Đã dùng {0} / {1}" },
                { "Hardware.Live", "TRỰC TIẾP" },
                { "Hardware.Open.Accessible", "Mở theo dõi phần cứng" },
                { "Hardware.Minimize.Accessible", "Thu nhỏ cửa sổ theo dõi phần cứng" },
                { "Hardware.Close.Accessible", "Đóng cửa sổ theo dõi phần cứng" },
        ...
        private static Dictionary<string, string> CreateJapanese()
        {
            return new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                { "Tray.HardwareMonitor", "ハードウェア モニター" },

                { "Hardware.WindowTitle", "ハードウェア モニター — Quick Controls" },
                { "Hardware.Title", "ハードウェア モニター" },
                { "Hardware.Intro", "パフォーマンスと温度をリアルタイムで確認" },
                { "Hardware.Cpu", "プロセッサ" },
                { "Hardware.Gpu", "グラフィックス" },
                { "Hardware.Memory", "メモリ" },
                { "Hardware.Storage", "ストレージ" },
                { "Hardware.Usage", "使用率" },
                { "Hardware.Temperature", "温度" },
                { "Hardware.TemperatureUnavailable", "報告なし" },
                { "Hardware.NotDetected", "検出されません" },
                { "Hardware.WarmingUp", "ハードウェア データを読み取り中…" },
                { "Hardware.History", "60 秒の履歴" },
                { "Hardware.LastUpdated", "{0} に更新" },
                { "Hardware.SensorNote", "温度はハードウェアとドライバーが対応している場合に表示されます" },
                { "Hardware.MemoryUsed", "{1} 中 {0} を使用" },
                { "Hardware.Live", "ライブ" },
                { "Hardware.Open.Accessible", "ハードウェア モニターを開く" },
                { "Hardware.Minimize.Accessible", "ハードウェア モニターを最小化" },
                { "Hardware.Close.Accessible", "ハードウェア モニターを閉じる" },
        ...
        private static Dictionary<string, string> CreateSimplifiedChinese()
        {
            return new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                { "Tray.HardwareMonitor", "硬件监视器" },

                { "Hardware.WindowTitle", "硬件监视器 — Quick Controls" },
                { "Hardware.Title", "硬件监视器" },
                { "Hardware.Intro", "实时查看性能和温度" },
                { "Hardware.Cpu", "处理器" },
                { "Hardware.Gpu", "图形处理器" },
                { "Hardware.Memory", "内存" },
                { "Hardware.Storage", "存储" },
                { "Hardware.Usage", "使用率" },
                { "Hardware.Temperature", "温度" },
                { "Hardware.TemperatureUnavailable", "未报告" },
                { "Hardware.NotDetected", "未检测到" },
                { "Hardware.WarmingUp", "正在读取硬件数据…" },
                { "Hardware.History", "60 秒历史" },
                { "Hardware.LastUpdated", "更新于 {0}" },
                { "Hardware.SensorNote", "硬件和驱动程序支持时会显示温度" },
                { "Hardware.MemoryUsed", "已使用 {0} / {1}" },
                { "Hardware.Live", "实时" },
                { "Hardware.Open.Accessible", "打开硬件监视器" },
                { "Hardware.Minimize.Accessible", "最小化硬件监视器" },
                { "Hardware.Close.Accessible", "关闭硬件监视器" },
        ...
        private static Dictionary<string, string> CreateFrench()
        {
            return new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                { "Tray.HardwareMonitor", "Moniteur matériel" },

                { "Hardware.WindowTitle", "Moniteur matériel — Quick Controls" },
                { "Hardware.Title", "Moniteur matériel" },
                { "Hardware.Intro", "Performances et températures en temps réel" },
                { "Hardware.Cpu", "Processeur" },
                { "Hardware.Gpu", "Carte graphique" },
                { "Hardware.Memory", "Mémoire" },
                { "Hardware.Storage", "Stockage" },
                { "Hardware.Usage", "UTILISATION" },
                { "Hardware.Temperature", "Température" },
                { "Hardware.TemperatureUnavailable", "Non signalée" },
                { "Hardware.NotDetected", "Non détecté" },
                { "Hardware.WarmingUp", "Lecture des données matérielles…" },
                { "Hardware.History", "Historique de 60 secondes" },
                { "Hardware.LastUpdated", "Mis à jour à {0}" },
                { "Hardware.SensorNote", "La température s’affiche si le matériel et le pilote la prennent en charge" },
                { "Hardware.MemoryUsed", "{0} utilisés sur {1}" },
                { "Hardware.Live", "DIRECT" },
                { "Hardware.Open.Accessible", "Ouvrir le moniteur matériel" },
                { "Hardware.Minimize.Accessible", "Réduire le moniteur matériel" },
                { "Hardware.Close.Accessible", "Fermer le moniteur matériel" },
